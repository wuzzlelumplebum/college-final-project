...
<!-- Page header -->
		<div class="page-title d-flex">
			<h4><span class="font-weight-semibold">Home</span> - Pembayaran DP Tagihan</h4>
...
<div class="content">
	...
	<div class="card">
		...
		<div class="card-body">
			<form id="form_payment" class="form-validate-jquery" action="{{ route('paymentclients.store')}}" method="post">
				@csrf
				<fieldset class="mb-3">
					<legend class="text-uppercase font-size-sm font-weight-bold">Data Payment</legend>
					<input type="hidden" name="tagihan_id" value="{{ $dptagihan }}">
					<div class="form-group row">
						<label class="col-form-label col-lg-2">Klien</label>
						<div class="col-lg-10">
                            <input type="hidden" name="user_id" value="{{ $user->id }}">
							<input type="text" name="nama" class="form-control border-teal border-1" value="{{$user->nama}}" readonly>
						</div>
					</div>
                    <div class="form-group row">
						<label class="col-form-label col-lg-2">Tagihan</label>
						<div class="col-lg-10">
                            <input type="hidden" name="rdtagihan" value="2">
                            <input type="text" name="tagihan" id="tagihan_id" class="form-control border-teal border-1" data-jmlbayar="{{ $dptagihan->jml_terbayar }}" data-tagihan="{{$dptagihan->total}}" value="{{ $dptagihan->invoice }} (Rp {{number_format($dptagihan->total, 0, ',', ',')}})" readonly>
                            <input type="hidden" name="rekap_dptagihan_id" id="rekap_dptagihan_id" value="{{ $dptagihan->id }}">
						</div>
					</div>
					<div class="form-group row">
				...
				<div class="text-right">
                    <a href="{{ url('/dptagihanuser') }}" class="btn bg-slate"><i class="icon-undo2 mr-2"></i>Kembali</a>
...
<script>
    ...
    var id = $('#rekap_tagihan_id').val();
    var tagih = $('#tagihan_id').data('tagihan');
    var bayar = $('#tagihan_id').data('jmlbayar');
	...
	$('#tertulis').keyup(function() {
		...
		$('#tertulis').val(harga);
		var total = $("#tagihan_id").data('tagihan');
		var bayar = $("#tagihan_id").data('jmlbayar');
